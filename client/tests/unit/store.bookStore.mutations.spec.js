...
describe('mutations', () => {
    it('adds a book to "state.books" array using "pushBookToUserList" mutation', () => {
        // mock state
        const state = { books: [] }
        // apply mutation
        const book = ['isbn','title', 'coverPath', 
            'author', 'publisher', 'publishDate']
        
        bookStore.mutations.pushBookToUserList(state, book)
    ...
    it('removes a book from "books.store" using the "removeBookFromUserList" mutation', () => {
        ...
        bookStore.mutations.removeBookFromUserList(state, 'isbn')
        expect(state.books).toEqual([])
    })
